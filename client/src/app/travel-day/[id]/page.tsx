  ...
  return (
    <div className="flex-1 flex flex-col justify-between relative overflow-hidden h-screen bg-[#090b11]">
      ...
      <div className="flex items-center gap-3 px-6 pt-6 pb-4 border-b border-white/5 bg-[#090b11]/80 backdrop-blur-md z-10">
        ...
        <div>
          <h1 className="text-xs font-bold text-white font-outfit truncate max-w-[180px]">
            {plan?.title || '旅行当日チャット'}
          </h1>
          <span className="text-[9px] text-emerald-400 font-bold block tracking-wider uppercase flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          >
            ...
            <div className="flex items-center gap-1.5 text-[9px] text-slate-500 font-bold">
              {msg.type === 'user' ? (
                ...
                <span className="text-indigo-400">システム</span>
              ) : msg.type === 'error' ? (
                <span className="text-rose-400">エラー</span>
              ) : (
            ...
            {msg.places && msg.places.length > 0 && (
              <div className="w-full space-y-2 mt-2">
                {msg.places.map((place: any, pIdx: number) => (
                  <div
                    key={pIdx}
                    className="bg-white/5 border border-white/5 rounded-2xl p-3.5 space-y-1.5 shadow-md flex flex-col justify-between"
                  >
                    <div>
                    ...
                    {msg.citations &&
                      msg.citations.find((c: any) => c.title?.includes(place.name)) && (
                        <a
                          ...
                          className="self-end text-[9px] text-emerald-400 hover:text-emerald-300 font-semibold flex items-center gap-0.5 mt-2"
                        >
        ...
        {loading && (
          <div className="flex flex-col gap-2 max-w-[85%] items-start">
            <span className="text-[9px] text-emerald-400 font-bold">当日アシスタント...</span>
      ...
      <div className="px-6 py-2 flex gap-2 overflow-x-auto whitespace-nowrap bg-[#090b11] border-t border-white/5">
        {QUICK_SUGGESTIONS.map((s, idx) => (
          <button
            key={idx}
            onClick={() => handleSend(s)}
            disabled={loading}
            className="text-[10px] font-medium text-slate-300 border border-white/10 hover:border-white/20 bg-white/5 rounded-full px-3 py-1.5 cursor-pointer transition-all active:scale-95"
          >
            {s}
          </button>
        ))}
      ...
      {/* Message input */}
      <div className="p-4 bg-[#090b11] z-10">
