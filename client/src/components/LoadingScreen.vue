<template>
  <div class="wizard-screen flex flex-col items-center justify-center h-full text-slate-700 p-6 pb-safe">
    <div class="text-lg font-semibold mb-2 tracking-tight">{{ titleText }}</div>
    <div class="text-xs text-slate-500 mb-10">{{ subtitleText }}</div>
    ...
    <div class="ai-thinking-container">
      ...
      <div class="brain-center">
        <i class="mdi mdi-brain" aria-label="AI brain thinking"></i>
      </div>
    </div>
  </div>
</template>
...
const props = defineProps({
  titleText: { type: String, default: 'AIが旅行プランを生成中...' },
  subtitleText: { type: String, default: '最適なプランを考えています。少しお待ちください。' }
})
</script>
...
.node-4 {
  ...
  bottom: 40px;
  left: 40px;
  animation: float-4 3.8s ease-in-out infinite 0.8s;
}
...
.node-5 {
  ...
  top: 60px;
  left: 30px;
  animation: float-5 3.2s ease-in-out infinite 1.2s;
}
...
.node-6 {
  ...
  top: 80px;
  right: 45px;
  animation: float-6 4.2s ease-in-out infinite 0.3s;
}
...
.node-7 {
  ...
  bottom: 70px;
  left: 80px;
  animation: float-7 3.7s ease-in-out infinite 1.5s;
}
...
.line-2 {
  ...
  animation: pulse-line 2.5s ease-in-out infinite 0.5s;
}
...
.line-3 {
  ...
  animation: pulse-line 2.2s ease-in-out infinite 1s;
}
...
.line-4 {
  ...
  animation: pulse-line 2.8s ease-in-out infinite 1.3s;
}
...
.brain-center .mdi { font-size: 40px; filter: drop-shadow(0 0 8px rgba(59, 130, 246, 0.3)); }
...
@keyframes float-1 {
  ...
  75% {
    transform: translate(-12px, -5px) scale(1.05);
    opacity: 0.9;
  }
}
...
@keyframes float-5 {
  ...
  50% {
    transform: translate(-15px, 8px) scale(1.3);
    opacity: 1;
  }
}
