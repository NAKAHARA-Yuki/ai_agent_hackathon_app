// Mock version of apiClient for testing

let mockEnv = {
  VITE_USE_MOCK: 'false'
}

export function __setMockEnv(env) {
  mockEnv = { ...mockEnv, ...env }
}

function getUseMock() {
  return mockEnv.VITE_USE_MOCK === 'true'
}
...
function randomId(){
  return global.crypto?.randomUUID?.() || `${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`
}
...
export async function agentChat({ message, user_id, session_id, authHeader }){
  if (!getUseMock()) {
    const data = await realFetch('/api/agent/chat', { 
      method:'POST', 
...
export async function listPlans(authHeader){
  if (!getUseMock()) {
    return await realFetch('/api/plans', { 
      headers:{ 'Content-Type':'application/json', ...(authHeader||{}) } 
    })
  }
...
export async function getActivePlan(authHeader){
  if (!getUseMock()) {
    return await realFetch('/api/active-plan', { 
      headers:{ 'Content-Type':'application/json', ...(authHeader||{}) } 
    })
  }
...
export async function planDetail(planId, authHeader) {
  if (!getUseMock()) {
    return await realFetch(`/api/plans/${planId}`, { 
      headers:{ 'Content-Type':'application/json', ...(authHeader||{}) } 
    })
  }
